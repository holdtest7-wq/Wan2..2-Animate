...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            media_key = next((k for k in ["images", "gifs", "videos"] if k in node_output), None)
            if media_key is not None:
                print(
                    f"worker-comfyui - Node {node_id} contains {len(node_output[media_key])} media file(s) (key={media_key})"
                )
